...
pub fn envconfig(input: TokenStream) -> TokenStream {
    ...
    let fields: Vec<Field> = field_nodes.iter()
    ...
    match body {
        ::syn::Body::Struct(variant_data) => {
            match variant_data {
                ...
                _ => panic!("Envconfig trait can not be derived from `{}` because it is not a struct.", name)
            }
        },
        _ => panic!("Envconfig trait can not be derived from `{}` because it is not a struct.", name)
    }
...
fn parse_field(field_node: syn::Field) -> Field {
    ...
    let var_name = match var_attr.value {
        ...
        _ => panic!("Envconfig: can not fetch value of var attribute for field `{}`", name)
    };
